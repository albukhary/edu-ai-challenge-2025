...
function plugboardSwap(c, pairs) {
  // If no pairs, return the character unchanged
  if (!pairs || pairs.length === 0) return c;

  // Check each pair for a match
...
class Enigma {
  constructor(rotorIDs, rotorPositions, ringSettings, plugboardPairs) {
    ...
    this.initialPositions = [...rotorPositions];
  ...
  process(text) {
    this.reset(); // Reset rotors to initial positions before processing
    return text
      .toUpperCase()
      .split("")
